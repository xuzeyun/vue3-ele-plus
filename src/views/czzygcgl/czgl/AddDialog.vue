<template>
  <el-dialog v-model="dialogVisible" :title="(dialogType === 1 ? '残渣' : '修改') + '登记'" width="30%" destroy-on-close draggable @close="close">
      ...
      <div class="g-box">
        ...
        >
          ...
          <el-form-item label="残渣类型">
            <el-select v-model="formInline.czlx" placeholder="请选择" @change="sfChange" filterable>
              <el-option v-for="(item, index) in sfOptions" :key="index" :label="item.name" :value="item.name" />
            </el-select>
          </el-form-item>
          <el-form-item label="重量(吨)">
            <el-input v-model="formInline.zl" placeholder="请输入"></el-input>
          </el-form-item>
          <el-form-item label="存储仓库">
            <el-select v-model="formInline.ccck" placeholder="请选择" @change="sfChange" filterable>
              <el-option v-for="(item, index) in sfOptions" :key="index" :label="item.name" :value="item.name" />
            </el-select>
          </el-form-item>          
        </el-form>
